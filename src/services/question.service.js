...
export default class QuestionService extends BasicService
    ...
    findAll(filter: Filter): Result {
        let fullUrl: string = FilterBuilder.buildUrlWithFilter(
            `${QUESTION_API}/find-all`,
            filter
        );
        if (RootScope.userId) {
            const api: string = QuestionService.buildURLWithToken(
                `${QUESTION_API}/find-all`
            );
    ...
    doApproveAnswer(questionId: Number, answerId: Number): Result {
        const fullUrl: string = QuestionService.buildURLWithToken(
            `${QUESTION_API}/approve-answer`
        );
        const data: Object = {
            id: questionId,
            answerId: answerId
    ...
    getPopularQuestionsInMonth(filter: Filter): Result {
        const fullUrl: string = FilterBuilder.buildUrlWithFilter(
            QUESTION_API,
            questionsFilterGenerator({ ...filter, order: 'numberOfViews DESC' })
        );
        return QuestionService.get(fullUrl, RootScope.axiosDefaultConfig);
    }
...
export async function getQuestions(params) {
    const response = await http.get(QUESTION_URL, { params });
