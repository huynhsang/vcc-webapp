...
const { accountService, questionService, subCategoryService } = CoreService;
...
const RightSideBar = () => {
    ...
    const getTopUsers = () => {
        ...
        accountService
            .getTopUsersWithTheHighestPoints(topUsersFilter)
            .then((result: Result) => {
                if (result.success) {
    ...
    const getTopPopularQuestions = () => {
        ...
        questionService
            .getPopularQuestionsInMonth(popularQuestionsFilter)
            .then((result: Result) => {
                if (result.success) {
                    setPopularQuestions(result.data);
                }
            });
    };

    const getQuestionsWithTopAnswers = () => {
        const qaFilter: Filter = FilterBuilder.buildPaginationFilter(
            null,
            0,
            5
        );
        questionService
            .getQuestionsWithTopAnswerInMonth(qaFilter)
            .then((result: Result) => {
                if (result.success) {
                    setQuestionsWithTopAnswers(result.data);
                }
            });
    };

    const getTopTrendingTags = () => {
        ...
        subCategoryService
            .getTrendingTags(trendingTagsFilter)
            .then((result: Result) => {
                if (result.success) {
