...
const TagsQuestion = ({ selectedTags, subCategories, next, previous }) => {
  const { t } = useTranslation();

  const [tagsEditted, setTagsEditted] = React.useState(selectedTags || []);
  const [suggestions, setSuggestion] = React.useState(subCategories || []);
  ...
  return (
    <section className="mt5 mb3">
      ...
      <div className="module p4 width-100">
        <p className="font-weight-700">{t('question_identify_your_tags')}</p>
        <p>{t('common_for_exemple')}:</p>
        <p className="m0">
          <span>
            <i className="fas fa-check" /> {t('question_include_tags_that')}
          </span>
          <br />
          <span>
            <i className="fas fa-times" /> {t('question_only_included_in')}
          </span>
        </p>
      </div>
      <div className="mt3">
      ...
      <div className="mt3 text-right">
        <button className="btn btn-light mr3" onClick={() => previous('Type')}>
          {t('common_previous_step')}
        </button>
        <button className="btn btn-primary" onClick={() => next(tagsEditted)}>
          {t('common_next')}
        </button>
      </div>
    </section>
